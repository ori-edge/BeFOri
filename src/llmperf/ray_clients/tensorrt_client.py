...
import numpy as np
import torch
from llmperf.utils.TensorRT import (
...
def parse_arguments(args=None):
    ...
    parser.add_argument(
        "--multimodal_input_file", type=str, help="Path to multimodal input file."
    )
    parser.add_argument(
        "--input_token_extra_ids",
        type=int,
        nargs="+",
        help="Input token extra ids for using p-tuning and KV Cache reuse together (only available with cpp session).",
        default=None,
    )
    parser.add_argument(
        "--input_token_extra_ids_file",
        type=str,
        help="CSV or Numpy file containing input token extra ids file. Alternative to text input (only available with cpp session).",
        default=None,
    )
    parser.add_argument(
        "--output_csv",
...
def parse_input_token_extra_ids(
    prompt_table_path,
    ...
    if prompt_table_path and kv_cache_enable_block_reuse:
        assert (
            input_token_extra_ids or input_token_extra_ids_file
...
def print_output(
    ...
    if output_csv is None and output_npy is None:
        for i in range(batch_size * num_return_sequences):
            ...
            for beam in range(num_beams):
                ...
                index_str = (
                    f"Text {batch_idx} Seq {seq_idx} Beam {beam}"
                    if num_return_sequences > 1
                    else f"Text {batch_idx} Beam {beam}"
                )
    ...
    if cum_log_probs is not None and output_cum_log_probs_npy is not None:
        ...
        cum_log_probs_outputs = np.array(
            cum_log_probs.cpu().contiguous(), dtype="float32"
        )
    ...
    if log_probs is not None and output_log_probs_npy is not None:
        ...
        log_probs_outputs = np.array(log_probs.cpu().contiguous(), dtype="float32")
        np.save(log_probs_file, log_probs_outputs)


def run_draft_target_model(
    batch_input_ids,
    args,
    runtime_rank,
    end_id,
    ...
    is_compute_acceptance_ratio = (
        logger.level == "verbose"
    )  # Only enable in verbose mode
    ...
    outputs["output_ids"] = torch.full(
        [input_batch_size, beam_width, max(max_seq_lengths)], end_id, dtype=torch.int32
    )
    for bs in range(input_batch_size):
        outputs["output_ids"][bs, :, : input_lengths[bs]] = batch_input_ids[bs]
    outputs["sequence_lengths"] = torch.full(
        [input_batch_size, beam_width], 0, dtype=torch.int32
    )
    outputs["context_logits"] = None
    outputs["generation_logits"] = torch.full(
        [input_batch_size, beam_width, max(max_seq_lengths), vocab_size],
        0,
        dtype=torch.float16,
    )
    ...
    while True:
        ...
        if use_logits:
            assert (
                "generation_logits" in draft.keys()
        ...
        for bs in range(batch_size):
            ...
            r = d_seq_len[bs]
            if l < r:
                d_ids[bs] = ([end_id] + draft["output_ids"][bs, 0, l:r].tolist())[
                    -draft_len:
                ]
                if use_logits:
        ...
        for bs in range(batch_size):
            ...
            if use_logits:
                outputs["generation_logits"][
                    index, 0, (l - input_lengths[bs]) : (r - input_lengths[bs])
                ] = (target["generation_logits"][bs][0, : (r - l)].detach().cpu())
            if is_compute_acceptance_ratio:
                n_draft_token[index] += len(d_ids[bs])
                n_accept_token[index] += sum(
                    d_ids[bs][i] == t_ids[bs][i]
                    for i in range(
                        min(
                            d_len[bs],
                            t_seq_len[bs] - prefix_len[bs],
                            max_seq_lengths[index] - prefix_len[bs],
                        )
                    )
                )
...
def main(args):
    ...
    if args.draft_target_model_config is not None:
        assert (
            args.draft_engine_dir is not None
        ), "Path to draft engine (--draft_engine_dir) must be specified."
        assert (
            args.engine_dir is not None
        ), "Path to target engine (--engine_dir) must be specified."
    ...
    input_token_extra_ids = parse_input_token_extra_ids(
        args.prompt_table_path,
        args.kv_cache_enable_block_reuse,
        args.input_token_extra_ids,
        args.input_token_extra_ids_file,
        args.max_input_length,
    )

    input_lengths = (
        [x.size(0) for x in decoder_input_ids]
        if is_enc_dec
        else [x.size(0) for x in batch_input_ids]
    )
    ...
    if (
        (not args.return_all_generated_tokens)
        and args.streaming
        and (args.num_beams > 1)
    ):
        logger.warning(
    ...
    if (
        args.draft_target_model_config is not None
    ):  # For Draft-Target-Model speculative decoding
        ...
        assert (
            not args.use_py_session
        ), "Only CPP session is supported in Draft-Target-Model."
        assert not is_enc_dec, "Only decoder model is supported in Draft-Target-Model."
        assert (
            args.num_beams == 1
        ), "Beam width > 1 is not supported in Draft-Target-Model."

        outputs = run_draft_target_model(
            batch_input_ids,
            args,
            runtime_rank,
            end_id,
        ...
        if (
            not args.streaming
        ):  # Unpack runner from the return value in No-Streaming mode
        ...
        if args.lookahead_config is not None:
            ...
            assert (
                len(args.lookahead_config) == 3
            ), "Lookahead needs [max_window_size, max_ngram_size, max_verification_set_size]"
        ...
        if not args.use_py_session:
            runner_kwargs.update(
                ...
                cross_kv_cache_fraction=(
                    args.cross_kv_cache_fraction if is_enc_dec else None
                ),
                enable_chunked_context=args.enable_chunked_context,
                multi_block_mode=args.multi_block_mode,
                cuda_graph_mode=args.cuda_graph_mode,
        ...
        with torch.no_grad():
            outputs = runner.generate(
                batch_input_ids=decoder_input_ids if is_enc_dec else batch_input_ids,
                encoder_input_ids=encoder_input_ids if is_enc_dec else None,
                encoder_input_features=encoder_input_features if is_enc_dec else None,
                encoder_output_lengths=encoder_output_lengths if is_enc_dec else None,
